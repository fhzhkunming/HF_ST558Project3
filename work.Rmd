...
# Fit a Lasso model
  lasso_fit <- train(Diabetes_binary ~ .,
                     ...
                     metric = "Accuracy" # Use logLoss metric for evaluation
                     )
...
# Predict probabilities on the test set 
  predicted_class <- predict(class_fit, newdata = test_set, type = "prob")

# Calculate LogLoss manually
  log_loss <- logLoss(test_set$Diabetes_binary, predicted_class$Diabetes)

# Check the model result
  class_fit

# Git the confusionMatrix and log loss value
  confusionMatrix(class_fit, newdata = test)
  log_loss
...
# Print and visualize the model results  
  ...
  plot(rf_fit)
# Predict probabilities on the test set 
  predicted_rf <- predict(rf_fit, newdata = test_set, type = "prob")
  
# Add a small constant value to the predicted probabilities
  predicted_rf$Diabetes <- pmax(predicted_rf$Diabetes, 1e-15)
  predicted_rf$Diabetes <- pmin(predicted_rf$Diabetes, 1 - 1e-15)

# Calculate LogLoss manually
  log_loss <- logLoss(test_set$Diabetes_binary, predicted_rf$Diabetes)
  log_loss
```
